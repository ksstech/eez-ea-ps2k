"""
ea_ps2k_driver.py  —  Driver for EA-PS2000B series power supplies (binary protocol)
...
class EaPs2k:
    ...
    def __init__(self, port: str = '/dev/ea-ps2k-port', timeout: float = 0.5):
        ...
        ----------
        port    : Serial port.
                  Linux/RPi:  '/dev/ea-ps2k-port'  (udev stable symlink)
        ...
        # NOTE: Tracking is controlled from the front panel only.
        # The PS2342-06B firmware does not accept remote tracking control
        # via object 54 — attempting to write bit 0x02 returns error 0x30.
        self._tracking: bool = False  # read-only cache from status byte
    ...
    def set_tracking(self, on: bool):
        """
        Tracking is controlled from the front panel only.

        The PS2342-06B firmware (V3.02) does not accept remote tracking
        control via object 54 — writing bit 0x02 returns error 0x30
        (upper limit exceeded). This method raises EaProtocolError.
        """
        raise EaProtocolError(
            'Tracking cannot be set remotely on this firmware. '
            'Use the front panel Tracking button.')

    def get_tracking(self) -> bool:
        """
        Read current tracking state from device status (object 71).
        Tracking can only be changed from the front panel.
        """
        d = self.get_actual(1)
