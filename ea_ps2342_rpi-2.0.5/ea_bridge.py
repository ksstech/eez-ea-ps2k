    ...
    def dispatch(self, raw: str) -> str:
        ...
        try:
            ...
            if upper.startswith('TRACK '):
                # Tracking cannot be set remotely on PS2342-06B firmware V3.02.
                # The device rejects writes to control byte bit 0x02 with
                # error 0x30 (upper limit exceeded).
                # Tracking state is read-only — use the front panel button.
                on = _on(_arg(cmd, 0))
                if on:
                    return 'ERR:Tracking cannot be set remotely (use front panel button)'
                else:
                    # TRACK OFF is a no-op — tracking state is read from device
                    return '0'
